...
public class PlayerController : SimpleGameStateObserver, IEventHandler{
	...
	void FixedUpdate () {
		...
		Vector3 verticalVect = verticalInput * m_TranslationSpeed * Time.fixedDeltaTime * transf.forward;
		Vector3 horizontalVect = horizontalInput * m_TranslationSpeed * Time.fixedDeltaTime * transf.right;
		//Quaternion qRot = Quaternion.AngleAxis(horizontalInput* m_RotationSpeed * Time.fixedDeltaTime, transf.up);

		m_Rigidbody.MovePosition(m_Rigidbody.position + verticalVect);
		m_Rigidbody.MovePosition(m_Rigidbody.position + horizontalVect);
